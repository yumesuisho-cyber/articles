...
def 一覧を出す(マスター, 順番, 使用履歴):
    ...
    for キー in sorted(順番, key=lambda k: 並び順(k, 使用履歴)):
        日付 = 使用履歴.get(キー)
        if not 日付:
            print(f"  {幅そろえ(表示名(マスター, キー), 幅)}{幅そろえ('― 未使用 ―', 16)}{幅そろえ('いつでも', 16)}0回  ★最優先")
            continue
        最終 = max(日付)
...
def main():
    ...
    if 未使用:
        print(f"未使用のカード {len(未使用)}枚: " + "、".join(表示名(マスター, k) for k in 未使用))
    if 照合できず:
