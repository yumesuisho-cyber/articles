...
#   python3 dev/tools/次のカード.py --ランダム       14日ルールを満たす中からランダムに引く
...
def 並び順(キー, 使用履歴):
    """この期間に登場しないカードを最優先、次に最終使用日が古い順

    リポジトリには朝活の全期間ぶんの記事があるわけではないので、
    「登場しない」＝「未使用」ではなく「この期間より前に使われた」の意味。
    どちらにせよ最も長く出ていないカードなので、先頭に置いてよい。
    """
    日付 = 使用履歴.get(キー)
    if not 日付:
        return (0, date.min, キー)  # この期間には登場しない
    return (1, max(日付), キー)


def 提案する(マスター, 順番, 使用履歴, 開始日, 日数, ランダム=False):
    予定 = {}
    行 = []
    for i in range(日数):
        ...
        if not 候補:
            ...
            continue
        # ランダムでも、母集団は「14日ルールを満たすカード」に限る。
        # 52枚から毎回引くと、1年で80回ほどルール違反が出るため。
        キー = random.choice(候補) if ランダム else min(候補, key=lambda k: 並び順(k, 使用履歴))
...
def 一覧を出す(マスター, 順番, 使用履歴):
    ...
    for キー in sorted(順番, key=lambda k: 並び順(k, 使用履歴)):
        日付 = 使用履歴.get(キー)
        if not 日付:
            print(f"  {幅そろえ(表示名(マスター, キー), 幅)}{幅そろえ('― 登場なし ―', 16)}{幅そろえ('いつでも', 16)}0回  ← 最優先")
            continue
        最終 = max(日付)
...
def main():
    引数 = sys.argv[1:]
    ランダム = any(a in ("--ランダム", "-r") for a in 引数)
    引数 = [a for a in 引数 if a not in ("--ランダム", "-r")]
    ...
    if 未使用:
        print(
            f"この期間の記事に登場しないカード {len(未使用)}枚: "
            + "、".join(表示名(マスター, k) for k in 未使用)
        )
        print(
            "（朝活はリポジトリにある期間より前から続いているので、"
            "それ以前に使われているはずです。\n"
            " 　このツールでは「いちばん長く出ていないカード」として最優先に置きます）"
        )
    if 照合できず:
    ...
    見出し = "ランダムに引く" if ランダム else "提案"
    print(f"\n── {日付表示(開始日)} から {日数}日分を{見出し} ──\n")

    行 = 提案する(マスター, 順番, 使用履歴, 開始日, 日数, ランダム)
    ...
    if ランダム:
        print(
            f"※ 同じカードは{インターバル日数}日以上あけるルールを満たすカードだけを"
            "母集団にして、その中からランダムに引いています。"
        )
        print("※ 実行するたびに結果は変わります。")
    else:
        print(
            f"※ 同じカードは{インターバル日数}日以上あけるルールを満たすものから、"
            "登場していない期間が長い順に選んでいます。"
        )
        print("※ ランダムに引きたいときは --ランダム を付けてください。")
